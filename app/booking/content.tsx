...
import { useAppointments, User } from "../context/appointment-context";
...
export default function BookingPage({ doctors }: BookingPageProps) {
  const { addAppointment } = useAppointments();
  const { user } = useAuth();
  const [isOpen, setIsOpen] = useState(false);
  const [submitted, setSubmitted] = useState(false);

  // formData stores full doctor object
  ...
  const handleChange = (
    ...
    if (name === "doctor") {
      const selectedDoctor = doctors.find((d) => d._id === value) || null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsOpen(false);
        setSubmitted(false);
        setFormData({ doctor: null, date: "", time: "", notes: "" });
      }, 2000);
    } catch (err) {
              ...
              <div className="w-full md:w-1/2 p-6 backdrop-blur-lg">
                ...
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                      <label className="block mb-1 font-medium">Qoshimcha eslatma</label>
                      <textarea
                        name="notes"
